...
func TestConvertHumanRequestToRawRequest_input(test *testing.T) {
	...
	humanRequest = helperCreateHumanRequest("input", "rgb 1")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "11")

	humanRequest = helperCreateHumanRequest("input", "rgb 2")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "12")

	humanRequest = helperCreateHumanRequest("input", "rgb 3")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "13")

	humanRequest = helperCreateHumanRequest("input", "rgb 4")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "14")

	humanRequest = helperCreateHumanRequest("input", "rgb 5")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "15")

	humanRequest = helperCreateHumanRequest("input", "rgb 6")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "16")

	humanRequest = helperCreateHumanRequest("input", "rgb 7")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "17")

	humanRequest = helperCreateHumanRequest("input", "rgb 8")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "18")

	humanRequest = helperCreateHumanRequest("input", "rgb 9")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "19")

	humanRequest = helperCreateHumanRequest("input", "video 1")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "21")

	humanRequest = helperCreateHumanRequest("input", "video 2")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "22")

	humanRequest = helperCreateHumanRequest("input", "video 3")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "23")

	humanRequest = helperCreateHumanRequest("input", "video 4")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "24")

	humanRequest = helperCreateHumanRequest("input", "video 5")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "25")

	humanRequest = helperCreateHumanRequest("input", "video 6")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "26")

	humanRequest = helperCreateHumanRequest("input", "video 7")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "27")

	humanRequest = helperCreateHumanRequest("input", "video 8")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "28")

	humanRequest = helperCreateHumanRequest("input", "video 9")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	...
	humanRequest = helperCreateHumanRequest("input", "digital 1")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "31")

	humanRequest = helperCreateHumanRequest("input", "digital 2")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "32")

	humanRequest = helperCreateHumanRequest("input", "digital 3")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "33")

	humanRequest = helperCreateHumanRequest("input", "digital 4")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "34")

	humanRequest = helperCreateHumanRequest("input", "digital 5")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "35")

	humanRequest = helperCreateHumanRequest("input", "digital 6")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "36")

	humanRequest = helperCreateHumanRequest("input", "digital 7")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "37")

	humanRequest = helperCreateHumanRequest("input", "digital 8")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "38")

	humanRequest = helperCreateHumanRequest("input", "digital 9")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "39")

	humanRequest = helperCreateHumanRequest("input", "storage 1")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "41")

	humanRequest = helperCreateHumanRequest("input", "storage 2")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "42")

	humanRequest = helperCreateHumanRequest("input", "storage 3")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "43")

	humanRequest = helperCreateHumanRequest("input", "storage 4")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "44")

	humanRequest = helperCreateHumanRequest("input", "storage 5")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "45")

	humanRequest = helperCreateHumanRequest("input", "storage 6")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "46")

	humanRequest = helperCreateHumanRequest("input", "storage 7")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "47")

	humanRequest = helperCreateHumanRequest("input", "storage 8")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "48")

	humanRequest = helperCreateHumanRequest("input", "storage 9")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "49")

	humanRequest = helperCreateHumanRequest("input", "network 1")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "51")

	humanRequest = helperCreateHumanRequest("input", "network 2")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "52")

	humanRequest = helperCreateHumanRequest("input", "network 3")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "53")

	humanRequest = helperCreateHumanRequest("input", "network 4")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "54")

	humanRequest = helperCreateHumanRequest("input", "network 5")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "55")

	humanRequest = helperCreateHumanRequest("input", "network 6")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "56")

	humanRequest = helperCreateHumanRequest("input", "network 7")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "57")

	humanRequest = helperCreateHumanRequest("input", "network 8")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
	assert.Equal(test, rawRequest.Parameter, "58")

	humanRequest = helperCreateHumanRequest("input", "network 9")
	rawRequest = convertHumanRequestToRawRequest(humanRequest)
	assert.Equal(test, rawRequest.Command, "INPT")
